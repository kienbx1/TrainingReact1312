...
const AddNewProduct = () => {
  ...
  return (
    <div className="bg-slate-100 p-3 flex flex-col mt-10">
      <form
        onSubmit={updateImage}
        className="p-10 flex flex-col justify-around w-full h-auto">
        <input
          type="file"
          name="file"
          accept="image/*"
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 h-auto gap-3 ">
          {(preview || []).map((url, index) => (
            <img
              className="rounded-lg w-[full] aspect-[1/2] object-contain rounded-lg"
              src={url}
              alt="Image Preview"
              key={index}
              onClick={() => {
